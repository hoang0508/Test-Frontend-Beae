  ...
  return (
    ...
    >
      <div className="w-[350px]">
        <p className="lg:w-[350px] lg:text-[21px] max-sm:text-[16px] max-sm:w-full leading-8 font-normal text-[#000000] mb-5">
          Graphite Artist Quality Fine Art Drawing and Sketching Pencils
          Replaceable Nib Pencils.
        </p>
        <div className="flex flex-col mb-9 max-sm:grid max-sm:grid-cols-2 max-sm:gap-x-5">
          {textDots &&
            textDots.map((item) => (
              <ul key={item.id}>
                <li className="list-disc max-sm:text-[13px]  font-semibold text-[18px] leading-10">
                  {item.text}
                </li>
              </ul>
            ))}
